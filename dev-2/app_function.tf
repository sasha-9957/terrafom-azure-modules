module "linux_function_app" {
  ...
  azurerm_linux_function_app_params = {
    linux_function_app1 = {
      ...
      service_plan_id     = module.azurerm_service_plan.service_plans["service_plan_linux"].id

      app_settings = {
        "APPINSIGHTS_INSTRUMENTATIONKEY"             = module.app_insights.application_insights["application_insights1"].instrumentation_key
        "EtlStudycastMover_StudycastExchangeStorage" = module.storage_account.storage_accounts["storage_account1"].primary_connection_string
        "EtlStudycastMover_TableNameToInsertReport"  = "public.study_cast2"
        "KEY_VAULT_NAME"                             = module.key_vault.key_vaults["key_vault1"].name
        ...
        "studycast_exchange_storage"                 = module.storage_account.storage_accounts["storage_account1"].primary_connection_string
        "timerSchedule"                              = "0 0 * * * *"
      }
      ...
      storage_account_name                           = module.storage_account.storage_accounts["storage_account1"].name
      storage_uses_managed_identity                  = null
      storage_key_vault_secret_id                    = null
      virtual_network_subnet_id                      = module.subnet.subnets["linux_subnet"].id
